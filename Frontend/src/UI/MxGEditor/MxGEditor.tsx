...
export default class MxGEditor extends Component<Props, State> {
  ...
  componentDidMount() {
    ...
    this.props.projectService.socket.on('nodeUpdated', (data) => {
      this.updateNodeFromServer(data);
    });

    this.props.projectService.socket.on('nodeDeleted', (data) => {
      this.deleteNodeFromServer(data);
    });

    this.props.projectService.socket.on('nodeCreated', (data) => {
      this.createNodeFromServer(data);
    });
  }
        ...
        if (me.currentModel) {
          for (let i = 0; i < me.currentModel.elements.length; i++) {
            ...
            if (element.id === uid) {
              ...
              // Emitir el evento de actualización del nodo
              me.emitUpdateNode(element);
            }
          }
        }
      }
          ...
          for (let i = 0; i < evt.properties.cells.length; i++) {
            ...
            if (uid) {
              ...
              if (element) {
                ...
                element.width = cell.geometry.width;
                element.height = cell.geometry.height;
                // Emitir el evento de creación del nodo
                me.emitCreateNode(element);
              }
            }
          }
        }
    ...
    graph.addListener(mx.mxEvent.CELL_CONNECTED, function (sender, evt) {
      try {
        ...
        if (!edge.value) {
          ...
          // Emitir el evento de creación del nodo
          me.emitCreateNode(relationship);
        }
      ...
      if (me.currentModel) {
        ...
        if (element) {
          ...
          if (relationship) {
            ...
            me.emitUpdateNode(relationship);
          }
        }
      }
    });
  ...
  emitUpdateNode(node: any) {
    this.props.projectService.socket.emit('updateNode', node);
  }

  emitDeleteNode(node: any) {
    this.props.projectService.socket.emit('deleteNode', node);
  }

  emitCreateNode(node: any) {
    this.props.projectService.socket.emit('createNode', node);
  }

  updateNodeFromServer(data: any) {
    if (this.graph && this.currentModel) {
      let element = this.props.projectService.findModelElementById(this.currentModel, data.id);
      if (element) {
        element.x = data.x;
        element.y = data.y;
        element.width = data.width;
        element.height = data.height;
        this.projectService_addUpdatedElementListener({ element: element });
      }
    }
  }

  deleteNodeFromServer(data: any) {
    if (this.graph && this.currentModel) {
      let element = this.props.projectService.findModelElementById(this.currentModel, data.id);
      if (element) {
        this.props.projectService.removeModelElementById(this.currentModel, data.id);
        let cell = MxgraphUtils.findVerticeById(this.graph, data.id, null);
        if (cell) {
  ...
  createNodeFromServer(data: any) {
    if (this.graph && this.currentModel) {
      this.currentModel.elements.push(data);
      this.projectService_addCreatedElementListener({ element: data });
    }
  }
